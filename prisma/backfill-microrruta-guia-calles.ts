// prisma/backfill-microrruta-guia-calles.ts
//
// Script de un solo uso: calcula y guarda la guía de calles de todas las
// microrrutas que ya existen en la base de datos — las creadas antes de
// que este cálculo se volviera automático en create()/updateGeom() (ver
// microrrutas-vias.util.ts, misma función que usa este script).
//
// Uso:
//   npx ts-node prisma/backfill-microrruta-guia-calles.ts
//
// Es seguro volver a correrlo más de una vez: cada llamada a
// calcularYGuardarGuiaCallesMicrorruta sobrescribe la guía existente de
// esa ruta con el cálculo actual, así que no se duplica nada.
...
import { Pool } from 'pg';
import { calcularYGuardarGuiaCallesMicrorruta } from '../src/microrrutas/utils/microrrutas-vias.util';
...
async function main() {
  ...
  console.log(
    `Se encontraron ${microrrutas.length} microrrutas. Calculando guía de calles...\n`,
  );
  ...
  for (const mr of microrrutas) {
    try {
      await calcularYGuardarGuiaCallesMicrorruta(prisma, mr.id);
      procesadas++;
      console.log(
